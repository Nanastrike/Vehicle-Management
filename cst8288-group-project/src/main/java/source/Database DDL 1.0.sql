...
-- 創建索引以提高查詢性能
CREATE INDEX idx_vehicle_id ON maintenance_tasks(vehicle_id);
CREATE INDEX idx_status ON maintenance_tasks(status);
CREATE INDEX idx_scheduled_date ON maintenance_tasks(scheduled_date);
...
-- 創建維護警報索引
CREATE INDEX idx_alert_vehicle_id ON maintenance_alerts(vehicle_id);
CREATE INDEX idx_alert_status ON maintenance_alerts(status);
